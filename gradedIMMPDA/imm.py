...
class IMM(Generic[MT]):
    ...
    def mix_states(
        ...
        mean = np.array([state.mean for mean in immstate.components])
        cov = np.array([state.cov for cov in immstate.components])
        mixed_states = list()

        for probability in mix_probabilities:
            mixed_mean, mixed_cov = gaussian_mixture_moments(probability, mean, cov)
            mixed_states.append(GaussParams(mixed_mean, mixed_cov))
    ...
    def mode_matched_prediction(
        ...
        modestates_pred = [filter.predict(state, Ts) for filter, state in zip(self.filters, mode_states)]
    ...
    def mode_matched_update(
        ...
        loglikelihood, gauss_params = list(), list()
        gauss_params = [filter.update(z, state) for filter, state in zip(self.filters, immstate.components)]
        loglikelihood = np.array([filter.loglikelihood(z, state) for filter, state in zip(self.filters, immstate.components)])
        updated_state = loglikelihood, gauss_params
    ...
    def update_mode_probabilities(
        ...
        numerator = mode_loglikelihood.T + np.log(immstate.weights)
        log_joint = logsumexp_own(numerator)
        update_mode_probabilities =  np.exp(numerator.T - log_joint.T)
    ...
    def update(
        ...
        mode_loglikelihood, updated_states = self.mode_matched_update(z, immstate, sensor_state=sensor_state)

        updated_weights = self.update_mode_probabilities(mode_loglikelihood, immstate, sensor_state=sensor_state)
        updated_states = self.sum(updated_states, axis=1)
